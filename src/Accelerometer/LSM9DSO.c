...
// need it in radians
//static long atan123(short y, short x)
//{
...
//Gyro_Init();	// This will "turn on" the gyro. Setting up interrupts, etc.
/*
//  setGyroODR(gODR); // Set the gyro output data rate and bandwidth.
//	setGyroScale(gScale); // Set the gyro range
*/
	// Accelerometer initialization stuff:
Accel_Init(); // "Turn on" all axes of the accel. Set up interrupts, etc.
/*
...
long findAngleMeas(long y, long x)
{
	...
	xTemp = atan2(y,x)*1000;
	xTemp = (xTemp*(572958+500))/100000;	
/*	xTemp = x;
//	yTemp = y;
//	
//	
...
void AccelStuff(unsigned long* ulDataTx, unsigned long* ulDataRx)
{
	...
	mag = sqrt((lTempAy*lTempAy)+(lTempAz*lTempAz));
	XLangleX = findAngleMeas(lTempAx,mag);
//	XLangleX = findAngleMeas(lTempAx,lTempAy);
//	
	mag = sqrt((lTempAx*lTempAx)+(lTempAz*lTempAz));
	XLangleY = findAngleMeas(lTempAy,mag);
//	XLangleY = findAngleMeas(lTempAy,lTempAx);
	
	mag = sqrt((lTempAx*lTempAx)+(lTempAy*lTempAy));
	XLangleZ = findAngleMeas(lTempAz,mag);
	
// Filter accel data
////////////////////////////////////////////////////////////
		lowPassFilterData(XM,XLangleX,XLangleY,XLangleZ);       
		XLangleX = (filtAx[0]+filtAx[1]+filtAx[2]+filtAx[3]+filtAx[4]+filtAx[5]+3)/6;
		XLangleY = (filtAy[0]+filtAy[1]+filtAy[2]+filtAy[3]+filtAy[4]+filtAy[5]+3)/6;
		XLangleZ = (filtAz[0]+filtAz[1]+filtAz[2]+filtAz[3]+filtAz[4]+filtAz[5]+3)/6;
////////////////////////////////////////////////////////////	
		
	printf("AngleX: "); Fixed_sDecOut22s(XLangleX); printf("%c",NEWLINE);	
	printf("AngleY: "); Fixed_sDecOut22s(XLangleY); printf("%c",NEWLINE);	
	printf("AngleZ: "); Fixed_sDecOut22s(XLangleZ); printf("%c",NEWLINE);		
	
	vectX = findAngleMeas(lTempAx,lTempAy);
	vectY = findAngleMeas(lTempAy,lTempAx);
	
	vectX /= 9; // this is vhat*cos(theta), the magnitude of unit vector in X direction
	vectY = 1000 - vectX; // this is vhat*sin(theta), the magnitude of unit vector in Y direction
